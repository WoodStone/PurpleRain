...
import no.vestein.purplerain.graphics.RainDropSprite

class RainDropRender extends EntityRenderer[RainDrop](classOf[RainDrop]) {

  override def renderEntity(e: Entity[_], camera: Camera, batch: Batch): Unit = {
    val entity = e.asInstanceOf[RainDrop]
    val sprite = new RainDropSprite(entity.size)
